...
found = False
for list_mo in re.finditer(r'<a\s+(?:[a-z]+="[^"]*"\s+)*href="(?:http://www.parliament.uk)?(/biographies[^"]+)"', list_page):
  found = True
  ...
  name_mo = re.search(r'<h1>([^<]+)</h1>', mp_page, re.DOTALL)
  if not name_mo:
  ...
  # Constituency
  constituency_mo = re.search(r'<label[^>]*>Constituency</label>\s+<div([^<]+)</div>', mp_page, re.DOTALL)
  ...
  westminster_mo = re.search(r'<h3 id="ctl00[^"]*">Parliamentary</h3>\s*<p>((?:[^<]+|<[^/]|</[^p])*)</p>', mp_page)
  ...
  constituency_mo = re.search(r'<h3 id="ctl00[^"]*">Constituency</h3>\s*<p>((?:[^<]+|<[^/]|</[^p])*)</p>', mp_page)
  if not constituency_mo:
  ...
  # Get website URL
  website_mo = re.search(r'<a id="ctl00_ctl00_FormContent_SiteSpecificPlaceholder_PageContent_ctlContactDetails_rptSocialMedia_ctl01_hypLink" rel="external" href="([^"]+)">', mp_page)
  website_url = website_mo.group(1) if website_mo else None
